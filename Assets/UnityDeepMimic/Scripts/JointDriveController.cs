...
public class BodyPart
{
    ...
    public void AlignJointToCurrentPose()
    {
        if (joint == null)
            return;
        ...
        {
            Quaternion jointSpaceRot =
                Quaternion.Inverse(joint.connectedBody.rotation) * rb.rotation;

            joint.targetRotation = jointSpaceRot;

            currentEularJointRotation = jointSpaceRot.eulerAngles;
        }
        else
        {
            joint.targetRotation = Quaternion.identity;
            currentEularJointRotation = Vector3.zero;
        }
    }
